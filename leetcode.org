...
We study the [[*Aymptotic notation][asymptotic]] efficiency of algorithms when we make the input size large enough that only the order of growth of the running time is relevant.
*** Asymptotic notation
:PROPERTIES:
:NOTER_PAGE: 64
:END:
...
| Algo           | Worst-case runing time | Average-case/expected running time |
|----------------+------------------------+------------------------------------|
| Insertion sort | $\Theta(n^2)$          | $\Theta(n^2)$                      |
| Merge          | $\Theta(n\lg n)$       | $\Theta(n\lg n)$                   |
| Heap           | $O(n\lg n)$            | -                                  |
| Quick          | $\Theta(n^2)$          | $\Theta(n\lg n)$ (expected)        |
| Counting       | $\Theta(k+n)$          | $\Theta(k+n)$                      |
| Radix          | $\Theta(d(k+n))$       | $\Theta(d(k+n))$                   |
| Bucket         | $\Theta(n^2)$          | $\Theta(n)$ (average-case)         |
*** Heapsort
:PROPERTIES:
:NOTER_PAGE: 172
:END:
